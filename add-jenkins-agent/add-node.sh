...
# ---->> remember to add IP address of master node here <<-----

''' public IP '''
ansible all -i '172.1.1.19,' -m copy -a "src=$node_name.groovy dest=/var/lib/jenkins/init.groovy.d" -k -u jenkins
...
cd playbooks
ansible-playbook -i inventory/hosts --limit master restartJenkins.yml -k -u jenkins --extra-vars "ansible_sudo_pass=jenkins"
...
sleep 35
ansible test -m file -a "path=/var/lib/jenkins/init.groovy.d/$node_name.groovy state=absent" -k -u jenkins
